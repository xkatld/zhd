...
class DownloadTask:
    ...
    def download_m3u8_segments(self):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=min(20, os.cpu_count() * 2)) as executor:  # 动态调整工人数
    ...
    def download_segment(self, session, ts_url, index, max_retries=3):
        ...
        if os.path.exists(ts_filename) and os.path.getsize(ts_filename) > 0:
            return True
        for attempt in range(max_retries):  # 添加重试逻辑
            try:
    ...
    def decrypt_ts_files(self, key_file, iv):
        ...
        if not self.check_external_tool("openssl"):  # 检查外部工具
    ...
    def merge_ts_files(self):
        ...
        if not self.check_external_tool("ffmpeg"):  # 检查外部工具
